...
class TestDatabaseConnectionPooling:
        ...
        try:
            ...
            def write_scores(thread_id: int, num_writes: int = 10) -> list[str]:
                ...
                for i in range(num_writes):
                    wallet = f"GTEST{thread_id:03d}{i:03d}" + "A" * 46  # Valid Stellar wallet format
    ...
    def test_multiple_stores_concurrent_writes(self):
        ...
        try:
            def worker_with_own_store(thread_id: int, num_writes: int = 5) -> list[str]:
                ...
                results = []
                for i in range(num_writes):
                    wallet = f"GMULT{thread_id:03d}{i:03d}" + "A" * 45  # Valid Stellar wallet format
            ...
            assert len(successful_writes) == expected_total, \
                f"Expected {expected_total} successful writes, got {len(successful_writes)}"
            
            assert len(failed_writes) == 0, f"Got unexpected failed writes: {failed_writes}"
            
            print(f"✅ Successfully completed {len(successful_writes)} writes from {num_threads} separate stores")
    ...
    def test_read_write_concurrency(self):
        ...
        try:
            ...
            def writer_worker(thread_id: int, num_writes: int = 20):
                ...
                for i in range(num_writes):
                    try:
                        ...
                        write_results.append(f"writer_{thread_id}_write_{i}_error_{type(e).__name__}")
            ...
            # Analyze results
            successful_reads = [r for r in results if "_read_" in r and (r.endswith("_success") or r.endswith("_notfound"))]
    ...
    def test_pool_configuration_from_env(self):
        ...
        try:
            ...
            try:
                ...
                assert engine.pool._max_overflow == 7, f"Expected max overflow 7, got {engine.pool._max_overflow}"
                assert engine.pool._timeout == 15, f"Expected timeout 15, got {engine.pool._timeout}"
...
def test_20_threads_concurrent_scoring():
    ...
    try:
        ...
        print(f"✅ ACCEPTANCE CRITERIA MET: {num_threads} threads wrote {scores_per_thread} scores each - all succeeded!")
