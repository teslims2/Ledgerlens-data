...
def _make_local_dataset(node_idx: int, n: int = 300) -> tuple[np.ndarray, np.ndarray]:
    """Each node gets a distinct random seed to simulate a different venue."""
    X, y = make_classification(
        n_samples=n,
        n_features=4,
        n_informative=3,
        n_redundant=0,
        random_state=node_idx * 7,
    )
    return X, y
...
def test_three_node_simulation_auc():
    ...
    X_test, y_test = make_classification(
        n_samples=500, n_features=4, n_informative=3, n_redundant=0, random_state=0
    )
    ...
    for i in range(n_nodes):
        ...
        X_trains.append(X)
        y_trains.append(y)
        m = SGDClassifier(loss="log_loss", max_iter=1, random_state=i)
    ...
    for _ in range(n_rounds):
        ...
        masked_deltas = []
        for i, (m, X, y) in enumerate(zip(models, X_trains, y_trains)):
            _set_w(m, global_w)
    ...
    eval_model = SGDClassifier(loss="log_loss", max_iter=1, random_state=0)
